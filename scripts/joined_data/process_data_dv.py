...
pumping_df = pd.read_csv(r'..\pumping_data\diamond_valley\Diamond Valley Data for Meters Database Pumpage Report.csv')
...
# # Build addictional units
# # final_df['wy_net_et_mm'] = (final_df['wy_et_m3'] - final_df['wy_precip_eff_m3'])/final_df['area_m2'] * 1000
# final_df['annual_net_et_mm'] = (final_df['annual_et_m3'] - final_df['annual_precip_eff_m3'])/final_df['area_m2'] * 1000
# final_df['pumping_mm'] = final_df['pumping_m3']/final_df['area_m2'] * 1000

# # final_df['wy_precip_eff_mm'] = (final_df['wy_precip_eff_m3'])/final_df['area_m2'] * 1000
# final_df['annual_precip_eff_mm'] = (final_df['annual_precip_eff_m3'])/final_df['area_m2'] * 1000

# ### Add cooks distance ###
# # Remove meter data
# cooks_df = final_df[final_df.meter_change == 0].copy()
# meter_df = final_df[final_df.meter_change == 1].copy()

# X = cooks_df['wy_net_et_mm']
# y = cooks_df['pumping_mm']

# # Create the model
# model = sm.OLS(y, X)

# # Fit the model
# results = model.fit(intercept=0)

# # Get cooks distance
# influence = results.get_influence()
# cooks = influence.cooks_distance[0]
# cooks_df['cooks_wy'] = cooks

# # Cooks outlier
# cooks_df['cooks_value'] = 4/len(cooks_df)
# cooks_df['cooks_outlier'] = (cooks_df['cooks_wy'] > cooks_df['cooks_value']).astype(int)

# # Merge with meter_df
# meter_df['cooks_value'] = 4/len(cooks_df)
# meter_df['cooks_outlier'] = 0
# meter_df['cooks_wy'] = 0

# final_df = pd.concat([cooks_df, meter_df], axis=0)


# final_df = final_df.loc[:, ['dri_field_id', 'all_app', 'wy_precip_eff_mm', 'annual_precip_eff_mm',
#                             'count_pump', 'count_et', 'area_m2',
#                             'year', 'wy_net_et_mm',
#                             'annual_net_et_mm', 'pumping_mm',
#                             'meter_change', 'cooks_wy', 'cooks_value',
#                             'cooks_outlier']]
